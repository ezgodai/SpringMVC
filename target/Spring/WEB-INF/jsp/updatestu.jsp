...
<head>
<meta charset="UTF-8">
	<link rel="icon" href="${pageContext.request.contextPath}/logo/me.ico" type="image/x-icon">
<title>学生信息</title>
</head>
<body>
	<form action="${ pageContext.request.contextPath }/submitstu.action"
		method="post" enctype="multipart/form-data">
		<input type="hidden" name="student.id" value="${stu.id }">
		<table>
			<tr>
				<td>ID：</td>
				<td><div title="不可修改の值">${stu.id }</div></td>
			</tr>
			<tr>
				<td>姓名：</td>
			...
			<tr>
				...
				<td><input type="checkbox" name="student.hobby" value="玩"
					...
					<c:if test="${fn:contains(stu.hobby,'睡' ) }"> checked </c:if>>睡</td>
			</tr>
			<tr>
				<td>备注：</td>
